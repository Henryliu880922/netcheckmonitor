...
- Fork 初始公開版本。
- 專案升級至 .NET 10。
- 建立跨平台（Windows／macOS）解決方案架構。
- 新增實驗性的命令列介面（CLI）。
- 新增 Core 與 CLI 的自動化單元測試。
- 建立獨立的版本編號機制。

如需查看 Fork 之前的更新內容，請參閱原始專案的 [NetCheckMonitor 0.9.8 更新說明](docs/RELEASE_NOTES_0.9.8.md)。
...
NetCheckMonitor 也包含一個實驗性的命令列介面，可用於自動化與腳本操作。
